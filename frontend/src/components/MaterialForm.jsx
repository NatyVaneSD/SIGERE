...
const MaterialForm = ({
  ...
  const [prateleiraError, setPrateleiraError] = useState("");

  const handlePrateleiraChange = (e) => {
    const { value } = e.target;
    // Validação para permitir apenas letras e números.
    if (/^[a-zA-Z0-9]*$/.test(value)) {
      setPrateleiraError("");
      handleMaterialChange(index, { prateleira: value });
    } else {
      setPrateleiraError("Apenas letras e números são permitidos.");
      handleMaterialChange(index, { prateleira: value });
    }
  };
        ...
        <Col>
          <Form.Group controlId={`prateleira-${index}`}>
            ...
            <Form.Control
              ...
              onChange={handlePrateleiraChange}
              isInvalid={!!prateleiraError}
            />
